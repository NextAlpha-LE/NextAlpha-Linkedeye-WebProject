...
function iconconnect(ip) {
    isToBeConnect = {}[true];
    connectsiteAdpWebSocket(adpobj[ip].ws.url, adpobj[ip].id, 0)
}

function connectsiteAdpWebSocket(wsUrl, wsiteName, tries, adp) {
    ...
    try {
        if (window.WebSocket) {
            ...
            var on_err = function (error) {
                ...
                document.getElementById(wsiteName + 'alast-conn').innerText = "Lastconnect : " + bodlastreconnect
                ...
                if (networkStatus === 'online') {
                    if (astompClient.connectionTries >= 10) {
                        ...
                        if (isToBeConnect = {}[true]) {
                            ...
                            connectsiteAdpWebSocket(astompClient.ws.url, astompClient.id, astompClient.connectionTries)
                        }
                        //console.log("astompClient.connectionTries --> " + astompClient.connectionTries)
                        // isToBeConnect = false;
                    }
