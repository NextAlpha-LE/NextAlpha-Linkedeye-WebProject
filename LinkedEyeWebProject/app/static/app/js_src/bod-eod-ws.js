...
function iconconnect(ip) {
    isToBeConnect = {}[true];
    connectWebSocket(bodobj[ip].ws.url, bodobj[ip].id, 0)
}

function connectWebSocket(wsUrl, wsiteName, tries, bdp) {
    ...
    try {
        if (window.WebSocket) {
            ...
            iconhtml += '</tbody>';
            iconhtml += '</table>';
            iconhtml += ' <p class="tooltiptexting" id="' + wsiteName + 'last-conn"></p>'
            ...
            var on_err = function (error) {
                ...
                $('#bsitesname').empty()
                $('#bsitesname').append(iconhtml)*/
                document.getElementById(wsiteName + 'status-conn').innerText = 'False(' + bstompClient.connectionTries + ')'
                ...
                if (networkStatus === 'online') {
                    if (bstompClient.connectionTries >= 10) {
                        ...
                        $('#bsitesname').empty()
                        $('#bsitesname').append(iconhtml)*/
                        document.getElementById(wsiteName + 'status-conn').innerText = 'Trying(' + bstompClient.connectionTries + ')'
                        ...
                        if (isToBeConnect = {}[true]) {
                            connectWebSocket(bstompClient.ws.url, bstompClient.id, bstompClient.connectionTries)
                        }
