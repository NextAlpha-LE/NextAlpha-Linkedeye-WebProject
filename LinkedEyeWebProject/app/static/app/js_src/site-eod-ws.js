...
function iconconnect(ip) {
    isToBeConnect = {}[true];
    connectsiteEodWebSocket(eodobj[ip].ws.url, eodobj[ip].id, 0)
}

function connectsiteEodWebSocket(wsUrl, wsiteName, tries, eod) {
                ...
                if (networkStatus === 'online') {
                    if (estompClient.connectionTries >= 10) {
                        ...
                        if (isToBeConnect = {}[true]) {
                            connectsiteEodWebSocket(estompClient.ws.url, estompClient.id, estompClient.connectionTries)
                        }
