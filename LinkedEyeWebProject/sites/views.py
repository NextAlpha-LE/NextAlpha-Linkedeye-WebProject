...
from sites.models import SiteModel, LocationModel, CountryModel, StateModel
from django.contrib.auth.decorators import login_required
        ...
        try:
            if parsed_json["operation"] == 'add':
                ...
                if SiteModel.objects.filter(sitename = sitename).exists():
                    ...
                    obj = SiteModel(sitename = sitename, location=parsed_json['location'], websocket_url=parsed_json['websocketurl'], entity_host=parsed_json['entityhost'], entity_port=parsed_json['entityport'], is_URLSecured=parsed_json['isURLSecured'], analytics_Prefix_URL=parsed_json['prefixurl'], redis_host=parsed_json['redishost'],redis_port=parsed_json['redisport'], is_enable = True, redmine_url=parsed_json['redmineurl'], prometheus_url=parsed_json['prometheusurl'],le_url=parsed_json['leurl'], lat=parsed_json['lat'], lng=parsed_json['lng'])
    ...
    try:
        if request.method == 'POST':
            ...
            if request.GET["type"] == 'clicksite':
                cursor.execute("select site.* from site INNER JOIN user_sites on (user_sites.site_id = site.id) where (user_sites.user_id='%s' and site.is_enable=%s and site.sitename='%s')" %(user_id, True,request.GET["site"]))
                resultList = fetchall(cursor)
                response['data'] = resultList
            elif request.GET["type"] == 'userbased':
                if request.GET["isOnlyEnabled"] == 'true':
                    cursor.execute("select site.* from site INNER JOIN user_sites on (user_sites.site_id = site.id) where (user_sites.user_id='%s' and site.is_enable=%s)" %(user_id, True))
                else:
                    cursor.execute("select site.* ,user_sites.is_enable from site INNER JOIN user_sites on (user_sites.site_id = site.id) where (user_sites.user_id='%s')" %(user_id))
                resultList = fetchall(cursor)
                response['data'] = resultList
            elif request.GET["type"] == 'locationbased':
                location = request.GET['location']
                cursor.execute("select site.* from site INNER JOIN user_sites on (user_sites.site_id = site.id) where (user_sites.user_id='%s' and site.is_enable=%s and site.location='%s')" %(user_id, True,location))
                resultList = fetchall(cursor)
                response['data'] = resultList
            else:
                temp_list = []
