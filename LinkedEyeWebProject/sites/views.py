        ...
        try:
            if parsed_json["operation"] == 'add':
                ...
                if SiteModel.objects.filter(sitename = sitename).exists():
                    ...
                    logger.debug("sites views: %s", obj)
                    obj.save()
    ...
    try:
        if request.method == 'POST':
            ...
            if request.GET["type"] == 'clicksite':
                ...
                cursor.execute(
                    "select site.* from site INNER JOIN user_sites on (user_sites.site_id = site.id) where (user_sites.user_id=%s and site.is_enable=%s and site.sitename=%s)",
                ...
                if request.GET["isOnlyEnabled"] == 'true':
                    cursor.execute(
                        "select site.* from site INNER JOIN user_sites on (user_sites.site_id = site.id) where (user_sites.user_id=%s and site.is_enable=%s)",
                        [user_id, True]
                    )
                else:
                    cursor.execute(
                        "select site.* ,user_sites.is_enable from site INNER JOIN user_sites on (user_sites.site_id = site.id) where (user_sites.user_id=%s)",
                        [user_id]
                    )
                resultList = fetchall(cursor)
                response['data'] = resultList
                ...
                cursor.execute(
                    "select site.* from site INNER JOIN user_sites on (user_sites.site_id = site.id) where (user_sites.user_id=%s and site.is_enable=%s and site.location=%s)",
